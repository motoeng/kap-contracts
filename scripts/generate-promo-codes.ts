...
const codes = []; // Use 'codes' directly as an array

for (let i = 0; i < 10000; i++) {
  const guid = randomUUID(); // Rename 'code' to 'guid'
  const hash = createHash("sha256").update(guid).digest();
  codes.push({
    index: i,
    guid, // Rename 'code' to 'guid'
...
async function processBatch() {
  ...
  for (let i = 0; i < 100 && current < codes.length; i++, current++) {
    const { guid, hashed_promo_code } = codes[current]; // Rename 'code' to 'guid'
    console.log(`Adding promo code ${guid} with hash ${hashed_promo_code}`); // Rename 'code' to 'guid'
